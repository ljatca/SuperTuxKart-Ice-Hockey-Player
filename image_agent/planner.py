...
# Directly use the CNNClassifier model from HW3 solution
...
def save_model(model):
    ...
    if isinstance(model, Planner):
        return save(model.state_dict(), path.join(path.dirname(path.abspath(__file__)), 'planner.pt'))
...
def load_model():
    ...
    from os import path
    r = Planner()
    r.load_state_dict(load(path.join(path.dirname(path.abspath(__file__)), 'planner.pt'), map_location='cpu'))
    return r
