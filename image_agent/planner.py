...
def spatial_argmax(logit):
    ...
    # print("spatial_argmax(logit): ", logit.size())
...
# Directly use the Detector model from HW4 solution
class Planner(torch.nn.Module):
    ...
    def __init__(self, layers=[16, 32, 64, 128], n_output_channels=2, kernel_size=3, use_skip=True):
        ...
        for i, l in enumerate(layers):
            conv_block = self.CNNClassifier.Block(c, l, kernel_size, 2)
            self.convs.append(conv_block)
            c = l

        for i, l in list(enumerate(layers))[::-1]:
            upconv_block = self.UpBlock(c, l, kernel_size, 2)
            self.upconvs.append(upconv_block)
            c = l
            if self.use_skip:
                c += skip_layer_size[i]
    ...
    def forward(self, x):
        ...
        #print("Upconv layers before reversal:")
...
def save_model(model):
    ...
    if isinstance(model, Planner):
        ...
        return model_scripted.save('planner.pt')
...
def load_model():
    ...
    model = load(path.join(path.dirname(path.abspath(__file__)), 'planner.pt'))
    return model
